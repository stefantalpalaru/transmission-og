...
EOF
    ...
    QT_UIC="$(which ${QT_UIC})"
    AC_CHECK_TOOLS([QT_RCC],[rcc rcc5 rcc-qt5],[false])
    QT_RCC="$(which ${QT_RCC})"
    AC_CHECK_TOOLS([QT_LRELEASE],[lrelease lrelease5 lrelease-qt5],[false])
    QT_LRELEASE="$(which ${QT_LRELEASE})"
    AC_CHECK_TOOLS([QT_LUPDATE],[lupdate lupdate5 lupdate-qt5],[false])
