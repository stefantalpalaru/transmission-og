...
static void utp_on_read(tr_peerIo* const io, unsigned char const* buf, size_t buflen)
{
...
static void utp_on_overhead(tr_peerIo* const io, bool const send, size_t const count, int type UNUSED)
{
...
static uint64 utp_callback(utp_callback_arguments* args)
{
    ...
    {
#ifdef TR_UTP_TRACE
        if (args->callback_type != UTP_ON_STATE_CHANGE || args->state != UTP_STATE_DESTROYING)
        {
            tr_logAddNamedDbg(
                "UTP",
                "[%p:%p] [%s] io is null! buf=%p, len=%zu, flags=%u, send/error_code/state=%d, type=%d",
                args->context,
                args->socket,
    ...
    case UTP_ON_STATE_CHANGE:
        utp_on_state_change(io, args->state);
        break;

    case UTP_ON_ERROR:
        utp_on_error(io, args->error_code);
        break;

    case UTP_ON_OVERHEAD_STATISTICS:
        utp_on_overhead(io, args->send != 0, args->len, args->type);
        break;
    }
