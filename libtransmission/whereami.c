...
// Custom overrides for libtransmission.
#include "utils.h"
#define WAI_MALLOC(size) tr_malloc(size)
#define WAI_REALLOC(p, size) tr_realloc(p, size)
#define WAI_FREE(p) tr_free(p)
...
int WAI_PREFIX(getExecutablePath)(char* out, int capacity, int* dirname_length)
{
    ...
    for (ok = false; !ok; ok = true)
    {
        ...
        else
        {
            ...
            while (1)
            {
                char const* separator = strchr(begin, ':');
                char const* end = separator ? separator : begin + strlen(begin);
