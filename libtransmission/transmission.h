...
typedef enum { TR_PREALLOCATE_NONE = 0, TR_PREALLOCATE_SPARSE = 1, TR_PREALLOCATE_FULL = 2 } tr_preallocation_mode;

typedef enum { TR_CLEAR_PREFERRED, TR_ENCRYPTION_PREFERRED, TR_ENCRYPTION_REQUIRED } tr_encryption_mode;
...
typedef tr_rpc_callback_status (
    *tr_rpc_func)(tr_session *session, tr_rpc_callback_type type, struct tr_torrent *tor_or_null, void *user_data);
...
typedef enum { TR_PORT_ERROR, TR_PORT_UNMAPPED, TR_PORT_UNMAPPING, TR_PORT_MAPPING, TR_PORT_MAPPED } tr_port_forwarding;

tr_port_forwarding tr_sessionGetPortForwarding(tr_session const *session);

typedef enum { TR_CLIENT_TO_PEER = 0, TR_UP = 0, TR_PEER_TO_CLIENT = 1, TR_DOWN = 1 } tr_direction;
...
typedef enum { TR_LOG_ERROR = 1, TR_LOG_INFO = 2, TR_LOG_DEBUG = 3, TR_LOG_FIREHOSE = 4 } tr_log_level;
...
typedef enum { TR_PARSE_OK, TR_PARSE_ERR, TR_PARSE_DUPLICATE } tr_parse_result;
...
typedef void (
    *tr_torrent_rename_done_func)(tr_torrent *torrent, char const *oldpath, char const *newname, int error, void *user_data);
...
enum { TR_LOC_MOVING, TR_LOC_DONE, TR_LOC_ERROR };
...
enum {
    ...
    TR_PRI_MIN = TR_PRI_LOW,
    TR_PRI_MAX = TR_PRI_HIGH
};
 ...
 */
typedef void (
    *tr_torrent_completeness_func)(tr_torrent *torrent, tr_completeness completeness, bool wasRunning, void *user_data);
