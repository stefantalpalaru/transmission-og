...
static tr_option opts[] = {
    ...
    { 'd', "downlimit", "Set the max download speed in " SPEED_K_STR " for the current torrent(s) or globally", "d", true, "<speed>" },
    ...
    { 953, "global-seedratio", "All torrents, unless overridden by a per-torrent setting, should seed until a specific ratio", "gsr", true, "ratio" },
    { 954, "no-global-seedratio", "All torrents, unless overridden by a per-torrent setting, should seed regardless of ratio", "GSR", false, NULL },
    ...
    { 'u', "uplimit", "Set the max upload speed in " SPEED_K_STR " for the current torrent(s) or globally", "u", true, "<speed>" },
    ...
    { 'w', "download-dir", "When used in conjunction with --add, set the new torrent's download folder. Otherwise, set the default download folder", "w", true, "<path>" },
...
static void printTrackersImpl(tr_variant *trackerStats)
{
    ...
    for (int i = 0; (t = tr_variantListChild(trackerStats, i)) != NULL; ++i)
    {
        ...
        if (tr_variantDictFindInt(t, TR_KEY_downloadCount, &downloadCount) &&
            ...
            tr_variantDictFindInt(t, TR_KEY_id, &id) && tr_variantDictFindBool(t, TR_KEY_isBackup, &isBackup) &&
